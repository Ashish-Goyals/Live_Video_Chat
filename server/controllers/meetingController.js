...
export const getUserSessions = async (req, res) => {
  try {
    ...
    // Fetch meetings where user is host OR listed participants

    const meetings =
      await sql`SELECT DISTINCT m.id,m.meeting_id,m.title,m.status,m.created_at,m.ended_at,m.host_id,u.name as host_name, u.email as host_email
        FROM meetings m
    ...
    const formattedMeeting = await Promise.all(
      meetings.map(async (m) => {
        ...
        const messages =
          await sql`SELECT id,sender_id,sender_name,text,timestamp FROM meeting_messages WHERE meeting_id = ${m.id} ORDER BY timestamp ASC`;
        return {
          id: m.id,
