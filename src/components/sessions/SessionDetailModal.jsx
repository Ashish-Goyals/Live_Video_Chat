      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-150">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isEnded ? 'bg-slate-100 text-slate-700' : 'bg-emerald-50 text-emerald-700'}`}
              >
          ...
          <button
            ...
            arial-label="Close session details"
          >
        ...
        <div className="flex border-b border-slate-100 px-6 bg-slate-50/50">
          ...
          <button
            ...
            className={`px-3 py-4 font-medium text-sm border-b-2 cursor-pointer transition-all ${activeTab === 'chat' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover-text-slate-900'}`}
          >
            Chat Transcript ({session.messages?.length || 0})
          </button>
          <button
            onClick={() => setActiveTab('participants')}
            className={`px-3 py-4 font-medium text-sm border-b-2 cursor-pointer transition-all ${activeTab === 'participants' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover-text-slate-900'}`}
          >
